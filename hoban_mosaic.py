...
def make_tile(page_number,n_tiles,row,column):
	path = "/pieces/diotima_quartet/arco_quartet.pdf"
	output_path = "/pieces/diotima_quartet/hoban_tiles/hoban%s_%s@%s.pdf" % (page_number,row,column)
...
source = PdfFileReader(open("/pieces/diotima_quartet/arco_quartet.pdf", "rb"))	
...
for m in range(montages):
	...
	os.chdir("/pieces/diotima_quartet/hoban_tiles/")
...
os.system("pdftk /pieces/diotima_quartet/hoban_tiles/pages/*.pdf cat output hoban_montages.pdf")
